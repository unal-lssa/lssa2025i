...
def generate_loadbalancer(name, backend=None):
    """
    Generate an Nginx load balancer configuration between frontend and backend.
    """
...
def generate_frontend(name, load_balancer):
    ...
    with open(os.path.join(path, 'app.js'), 'w') as f:
        f.write(textwrap.dedent(f"""
            ...
            const LOAD_BALANCER = 'http://{load_balancer}:80';
            app.get('/', async (req, res) => {{
                try {{
                    const response = await
                    axios.get(`${{LOAD_BALANCER}}/systems`);
            ...
            app.post('/create', async (req, res) => {{
                ...
                await axios.post(`${{LOAD_BALANCER}}/create`, {{ name }});
                res.redirect('/');
            }});
    ...
    for e in model.elements:
        if e.__class__.__name__ == 'Component':
            ...
            if e.type == 'backend':
                ...
                generate_frontend(e.name, load_balancer=loadbalancer_name)
            elif e.type == 'loadbalancer':
