...
def generate_balanceador(name, backends):

    path = f'skeleton/{name}'
    os.makedirs(path, exist_ok=True)

    upstream = '\n'.join([f'        server {b}:80;' for b in backends])
...
def apply_transformations(model):
    ...
    database_name = None
    backend_names = []
    balanceador_name = None

    for e in model.elements:
        if e.__class__.__name__ == 'Component':
            if e.type == 'backend':
                backend_name = e.name  # el primero que aparezca
                backend_names.append(e.name)
            elif e.type == 'database':
                database_name = e.name
            elif e.type == 'balanceador':
                balanceador_name = e.name
    ...
    for e in model.elements:
        if e.__class__.__name__ == 'Component':
            ...
            if e.type == 'database':
                ...
                target = balanceador_name if balanceador_name else backend_name
                generate_frontend(e.name, backend=target)
            elif e.type == 'balanceador':
                generate_balanceador(e.name, backends=backend_names)
