...
def apply_transformations(model):
    ...
    # Primer recorrido: identificar nombres clave
    for e in model.elements:
        if isinstance(e, type(model.elements[0])) and hasattr(e, 'type'):
            if e.type == 'backend':
                backend_name = e.name
                backend_targets.append(e.name)
            elif e.type == 'database':
                database_name = e.name
            elif e.type == 'load_balancer':
                load_balancer_name = e.name

    # Segundo recorrido: generar los componentes
    for e in model.elements:
        if e.__class__.__name__ == 'Component':
            ...
            if e.type == 'database':
                ...
                # El frontend se conecta al balanceador si existe
                target = load_balancer_name if load_balancer_name else backend_name
                generate_frontend(e.name, backend=target)

            elif e.type == 'load_balancer':
                generate_load_balancer(e.name, targets=backend_targets)
...
def generate_load_balancer(name, targets):
    ...
    backends_config = '\n'.join([
        f"    server {target}:80;" for target in targets
    ])
...
{backends_config}
    ...
    with open(os.path.join(path, 'default.conf'), 'w') as f:
        f.write(nginx_conf)
