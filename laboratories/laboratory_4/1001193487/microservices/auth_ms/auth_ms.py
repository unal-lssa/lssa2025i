	...
	if username and password:
		...
		if response.status_code == 200:
			...
			if user_data['password'] == password:
				# Generate JWT token
				token = jwt.encode({'username': username, 'id': user_data['id']}, SECRET_KEY, algorithm='HS256')
...
def register():
	...
	if username and password:
		response = requests.post(f'{db_url}/addUser', json={'id': id, 'username': username, 'password': password, 'role': role}, timeout=5)
