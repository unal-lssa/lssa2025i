...
def generate_docker_compose(
    components, replicas=None, load_balancer_config=None, db_types=None
):
    path = f"skeleton/"
    ...
    with open(os.path.join(path, "docker-compose.yml"), "w") as f:
        sorted_components = dict(
            sorted(
                components.items(),
                key=lambda item: 0 if item[1] == "database" or item[1] == "db" else 1,
            )
        )
        f.write("services:\n")
                        ...
                        if not backend_service:
                            for comp_name, comp_type in components.items():
                                if comp_type == "backend":
                                    if (
                                        comp_name in replicas
                                        and replicas[comp_name] > 1
                                    ):
        ...
        buckets = {
            name: ctype for name, ctype in components.items() if ctype == "bucket"
        }
        ...
        for name in buckets:
            ...
            f.write(f"    volumes:\n")
            f.write(
                f"      - ${{LOCALSTACK_VOLUME_DIR:-./volume}}:/var/lib/localstack\n"
            )
        ...
        for name in queues:
            ...
            f.write(f"      KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR: 1\n")
