    ...
    for e in model.elements:
        if e.__class__.__name__ == "Connector":
            if getattr(e, "from").name == "api_gateway_comp":
                ...
                conn_type = e.type
                targets = (
                    e.to if isinstance(e.to, list) else [e.to]
                ...
                for target in targets:
                    api_gateway_connections[source_component][conn_type].append(
                        target.name
                    )
...
def apply_transformations(model):
    ...
    for e in model.elements:
        if (
            e.__class__.__name__ == "Component"
            or e.__class__.__name__ == "StandardComponent"
        ):
    ...
    for e in model.elements:
        if (
            ...
            if e.type == "queue":
                ...
                print(f"Connections for queue {e.name}: {connections}")

                if connections and "kafka_connector" in connections:
                    for type_conn, target_name in connections.items():
                        if type_conn == "kafka_connector":
                            producer_connections[target_name] = e.name
                            print(f"Producer connection: {target_name} -> {e.name}")
    print(f"Producer connections: {producer_connections}")
    ...
    for e in model.elements:
        if (
            ...
            if e.type == "database" or e.type == "db":
                ...
                # Check if this backend is connected to a queue
                if e.name in producer_connections:
                    print("Generating producer for backend")
                    generate_backend(
                        e.name,
                        connections={"kafka_connector_reverse": e.name},
                    )
                else:
                    connections = component_connections.get(e.name, {})
                ...
                generate_frontend(
                    e.name,
                    backend=backend_service if backend_service else target_name,
                    connections=connections,
                )
            elif e.type == "cdn":
                generate_cdn(e.name)
            ...
            generate_database(
                e.name, e.databaseType if hasattr(e, "databaseType") else "mysql"
            )
