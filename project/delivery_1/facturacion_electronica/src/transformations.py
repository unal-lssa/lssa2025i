...
def generate_load_balancer(element, target):
    ...
    VALOR = ";\n".join(f"{target['name']}_{i}" + f":{target['port']}" for i in range(target['instances'])) + ";"
...
def apply_transformations(model):
    ...
    for component_type, elements in components.items():
        if component_type == "frontend":
            ...
            for element in elements:
                load_balancer_targets = get_load_balancer_target(element, connectors)
                generate_load_balancer(element, load_balancer_targets)
...
def get_templates_path(element_name: str) -> str:
    ...
    :return: str
    """
    templates_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "templates", element_name))
    ...
    if not os.path.exists(templates_dir):
        ...
        raise FileNotFoundError(f"El directorio de plantillas no existe: {templates_dir}")
...
def get_skeleton_path(element_name: str) -> str:
    ...
    :return: str
    """
    skeleton_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "skeleton", element_name))
    # Crear el directorio de salida si no existe
    if not os.path.exists(skeleton_dir):
        os.makedirs(skeleton_dir)
    return skeleton_dir
...
def get_load_balancer_target(element, connectors):
    ...
    target = list(filter(lambda conn: conn.type == 'lb_conn' and getattr(conn, "from").name == element.name, connectors))[0].to
